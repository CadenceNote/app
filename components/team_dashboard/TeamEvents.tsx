...
import { ExpandableEventCard } from "@/components/card/expandable-event-card"
...
function getDisplayName(participant: { name?: string; full_name?: string; email?: string }): string {
                            ...
                            <div className="space-y-3">
                                ...
                                {tasks?.filter(task => task.due_date && format(parseISO(task.due_date), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd')).length > 0 && (
                                    <div className="space-y-1">
                                        ...
                                        {tasks?.filter(task => task.due_date && format(parseISO(task.due_date), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd'))
                                            .map(task => (
                                                <ExpandableEventCard
                                                    key={task.id}
                                                    title={task.title}
                                ...
                                {meetings?.filter(meeting => format(parseISO(meeting.start_time), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd')).length > 0 && (
                                    <div className="space-y-1">
                                        ...
                                        {meetings?.filter(meeting => format(parseISO(meeting.start_time), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd'))
                                            .sort((a, b) => parseISO(a.start_time).getTime() - parseISO(b.start_time).getTime())
                                            .map(meeting => (
                                                <ExpandableEventCard
                                                    key={meeting.id}
                                ...
                                {(!tasks?.filter(task => task.due_date && format(parseISO(task.due_date), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd')).length) &&
                                    (!meetings?.filter(meeting => format(parseISO(meeting.start_time), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd')).length) && (
